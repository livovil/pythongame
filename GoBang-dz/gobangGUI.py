...
class GoBang(QWidget):
    ...
    def initUI(self, uname):
        ...
        self.setWindowTitle(uname)  # 窗口名称
        ...
        if e.button() == Qt.LeftButton and self.ai_down == True:
            ...
            if not i is None and not j is None:  # 棋子落在棋盘上，排除边缘
                ...
                if self.chessboard.get_xy_on_logic_state(i, j) == EMPTY:
                    ...
                    a = self.AI.finishSignal.connect(self.AI_draw)  # 结束线程，传出参数
                    self.AI.start()  # run
    ...
    def gameover(self, winner):
        if winner == BLACK:
            self.sound_win.play()
            reply = QMessageBox.question(self, '您赢了!', '再来一局?',
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        elif winner == WHITE:
            self.sound_defeated.play()
            reply = QMessageBox.question(self, '您输了!', '再来一局?',
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
